...
def main():
    ...
    translationDict = generateTokenDict(tokenizerModel)
    #save the translationDict
    if (output_file == None):
        print (translationDict)
    else:
        if not os.path.exists(output_file):
            os.makedirs(output_file)
        np.save(output_file + "/GroupDict.npy", translationDict)
    exit()


def generateTokenDict(tokenizerModel):
    initTokenGroups()
    return initilizeTokenTranslation(tokenizerModel)
...
def initilizeTokenTranslation(tokenizer):
    ...
    for i in range(1, len(tokenTranslation)+1):
        #print (str(i) + " " + str(tokenTranslation[i]))
        translationDict[i] = tokenTranslation[i-1]
